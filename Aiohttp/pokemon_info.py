...
async def get_pokeinfo_from_id(poke_id: int) -> PokeInfo:
    async with request_sessions.poke_session.get(f"{BASE_URL}/{poke_id}") as response:
        return pokeresp_to_info(await response.json())


async def get_pokeinfo_from_name(name: str) -> PokeInfo:
    async with request_sessions.poke_session.get(f"{BASE_URL}/{name}") as response:
        return pokeresp_to_info(await response.json())


async def get_pokeinfos_from_offset_limit(offset: int, limit: int) -> list[PokeInfo]:
    async with request_sessions.poke_session.get(
        f"{BASE_URL}", params={"offset": offset, "limit": limit}
    ) as response:
